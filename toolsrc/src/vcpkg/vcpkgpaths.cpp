...
namespace vcpkg
{
                                            ...
                                            const std::vector<std::string>* triplets_dirs)
    {
        ...
        const auto overriddenBuildtreesPath = System::get_environment_variable("VCPKG_BUILDTREES");
        if (auto obp = overriddenBuildtreesPath.get())
        {
            auto asPath = fs::u8path(*obp);
            if (!fs::is_directory(fs.status(VCPKG_LINE_INFO, asPath)))
            {
                Metrics::g_metrics.lock()->track_property("error", "Invalid VCPKG_BUILDTREES override directory.");
                Checks::exit_with_message(
                    VCPKG_LINE_INFO,
                    "Invalid buildtrees override directory: %s; "
                    "create that directory or unset VCPKG_BUILDTREES to use the default buildtrees location.",
                    asPath.u8string());
            }

            paths.buildtrees = fs::stdfs::canonical(std::move(asPath), ec);
            if (ec)
            {
                return ec;
            }
